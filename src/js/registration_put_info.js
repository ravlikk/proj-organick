...
function checkInputsFilled(showErrors = true) {
  ...
  const fields = [
    {
      input: name,
      message: "⚠️ <strong>Name is required</strong>",
      validate: (val) => val.trim() !== "",
    },
    {
      input: email,
      message: "⚠️ <strong>Email is required</strong>",
      validate: (val) => val.trim() !== "",
    },
    {
      input: password,
      message: "⚠️ <strong>Password is required</strong>",
      validate: (val) => val.trim() !== "",
    },
  ];
  ...
  if (valid && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim())) {
    if (showErrors)
      showError(email, "⚠️ <strong>Invalid email format</strong>");
    valid = false;
  }
  ...
  if (valid && password.value.trim().length < 6) {
    if (showErrors)
      showError(
        password,
        "⚠️ <strong>Password must be at least 6 characters</strong>"
      );
    valid = false;
  }
